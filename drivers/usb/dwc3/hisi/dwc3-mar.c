...
{
	if (!hisi_dwc3->core_ops) {
		usb_err("[USB.CP0] usb driver not setup!\n");
		return -ENODEV;
	}

	/* disble pipe clock use for DP4 and must use usb2 */
	hisi_dwc3->core_ops->disable_pipe_clock();
...
static void mar_cmd_tmo_dbg_print(struct hisi_dwc3_device *hisi_dwc3)
{
	...
	hisi_dwc3->core_ops->logic_analyzer_trace_set(0);
	mar_logic_analyzer_trace_print(hisi_dwc3);
	hisi_dwc3->core_ops->logic_analyzer_trace_set(0x17);
	mar_logic_analyzer_trace_print(hisi_dwc3);
	usb_dbg("-\n");
}
...
int hisi_usb_combophy_notify(enum phy_change_type type)
{
	...
	if (PHY_MODE_CHANGE_BEGIN == type) {
		ret = mar_usb31_core_enable_u3(hisi_dwc3);
