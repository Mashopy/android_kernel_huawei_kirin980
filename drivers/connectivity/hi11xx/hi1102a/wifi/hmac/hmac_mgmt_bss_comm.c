                ...
                oal_uint8           uc_status)
{
    ...
    if (OAL_PTR_NULL != pst_hmac_user)
    {
        ...
        if (WLAN_VAP_MODE_BSS_STA == pst_vap->st_vap_base_info.en_vap_mode)
        {
            ...
            {
                us_ba_size = hmac_btcoex_get_ba_size(pst_vap,pst_hmac_user);
                if (0 == us_ba_size)
                {
                    us_ba_size = (oal_uint16)pst_hmac_user_btcoex->uc_ba_size;
                }
                us_ba_param |= (oal_uint16)(us_ba_size << 6);   /* BIT6 */
            }
            else
            {
                us_ba_param |= (oal_uint16)(pst_addba_rsp->us_baw_size << 6);   /* BIT6 */
                ...
                oal_uint8                  *puc_payload)
{
    ...
    {
        OAM_WARNING_LOG2(pst_mac_vap->uc_vap_id, OAM_SF_BA, "{hmac_mgmt_rx_addba_rsp::addba rsp is received whne ba status is DMAC_BA_COMPLETE or uc_dialog_token wrong.tid[%d], status[%d], rsp dialog[%d], req dialog[%d]}",
...
oal_uint32 hmac_sa_query_interval_timeout(oal_void *p_arg)
{
    ...
    if (ul_passed >= mac_mib_get_dot11AssociationSAQueryMaximumTimeout(&pst_hmac_vap->st_vap_base_info))
    {
        ...
        if (OAL_SUCC != ul_relt)
        {
            OAM_ERROR_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout_etc::hmac_sa_query_del_user failed[%d].}", ul_relt);
    ...
    OAM_WARNING_LOG1(pst_hmac_vap->st_vap_base_info.uc_vap_id, OAM_SF_ANY, "{hmac_sa_query_interval_timeout_etc::SA query, trans_id %d.}",
        pst_hmac_user->st_sa_query_info.us_sa_query_trans_id);
...
OAL_STATIC oal_int32 hmac_mac2str(oal_uint8 *puc_str, oal_uint8 *puc_mac)
{
    ...
    /* ��κϷ��ж� */
    if (OAL_UNLIKELY((OAL_PTR_NULL == puc_str) || (OAL_PTR_NULL == puc_mac)))
    {
        OAM_ERROR_LOG0(0, OAM_SF_ANY, "{hmac_mac2str: input pointer is null!}");
        return 0;
    }
