...
oal_void oal_sdio_build_rx_netbuf_list_work(oal_work_stru *pst_work)
{
    ...
    if(OAL_UNLIKELY(NULL == pst_hcc_bus))
    {
    ...
    while(oal_netbuf_list_len(phead) < HCC_BUS_MEMALLOC_MAX)
    {
        ...
        if( NULL == netbuf)
        {
            oal_print_hi11xx_log(HI11XX_LOG_ERR,"%s error: oal_sdio_build_rx_netbuf_list_work: alloc netbuf is null",__FUNCTION__);
            break;
        }
...
failed_sdio_reg:
    ...
    hw_1102a_dsm_client_notify(DSM_WIFI_SDIO_PROBE_ATTACH_ERROR, "%s: failed to probe sdio[%s],ret=%d\n", __FUNCTION__, sdio_enum_err_str, ret);
#endif
    hi_wlan_power_set(0);
    return -OAL_EFAIL;
}
