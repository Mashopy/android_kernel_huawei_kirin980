...
static int kbase_gmc_page_decompress(struct tagged_addr *p, struct kbase_device *kbdev, u8 policy_id)
{
	...
	err = gmc_storage_get_page(kbdev->hisi_dev_data.kbase_gmc_device.storage, page, handle);
	if (err) {
...
static int kbase_gmc_page_compress(struct tagged_addr *p, struct kbase_device *kbdev)
{
	...
	handle = gmc_storage_put_page(kbdev->hisi_dev_data.kbase_gmc_device.storage, page);
	if (IS_ERR(handle)) {
		dma_addr_t dma_addr;
		dma_addr = kbase_gmc_dma_map_page(kbdev, page);
		if (dma_addr != as_phys_addr_t(*p)) {
		...
		struct tagged_addr *start, size_t pages_num)
{
	...
	for (i = 0; i < pages_num; i++) {
		...
		if (as_phys_addr_t(*p) && kbase_is_entry_compressed(*p)) {
			...
			gmc_storage_invalidate_page(kctx->kbdev->hisi_dev_data.kbase_gmc_device.storage,
...
static int kbase_gmc_decompress_alloc(struct kbase_mem_phy_alloc *alloc, s64 start_idx, size_t nr)
{
	...
	for (i = start_idx; i < start_idx + nr_size; i++) {
		...
		if (as_phys_addr_t(*tagged_page) && kbase_is_entry_compressed(*tagged_page)) {
			ret = kbase_gmc_page_decompress(tagged_page, alloc->imported.native.kctx->kbdev, alloc->lb_policy_id);
			if (ret) {
...
static int kbase_gmc_decompress_region(struct kbase_va_region *reg, u64 vpfn, size_t nr)
{
	...
	if(kctx->hisi_ctx_data.set_pt_flag)
	{
		...
		ret_insert = kbase_mmu_insert_pages(kctx->kbdev, &kctx->mmu, vpfn,
			&reg->cpu_alloc->pages[vpfn - reg->start_pfn], nr, reg->flags, kctx->as_nr,header_page_number);
	}
...
{
	struct kbase_hisi_device_data *hisi_dev = container_of(gmc_dev, struct kbase_hisi_device_data, kbase_gmc_device);
	struct kbase_device *kbdev = container_of(hisi_dev, struct kbase_device, hisi_dev_data);
